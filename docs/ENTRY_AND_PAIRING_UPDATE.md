...
- `SOLO`: явный выбор «Хочу найти партнёра»; после вопросов основной переход ведёт к карточке знакомств. Личное развитие и собственный профиль остаются отдельными доступными действиями.
- `EXISTING_PARTNER`: личная настройка, затем `/invite` для реального партнёра.
- Намерение не создаёт Pair. `personal.relationshipStatus` сохраняет заявленные отношения: `EXISTING_PARTNER` означает `in_relationship` и до связывания. Активная/приостановленная Pair также устанавливает этот статус; `hasPair` по-прежнему обозначает только фактическую Pair.
...
Обновление 2026-09-09: в `/profile` можно вставить код «Вместе», ключ или ссылку приглашения и принять приглашение на том же экране. Сначала выполняется проверка, затем показывается личность приглашающего и отдельное неотмеченное подтверждение. После принятия профиль показывает ожидание подтверждения владельцем. При связанной active/paused Pair вместо формы отображается текущая пара; ввод ключа не заменяет существующую связь. Требуемые entry/onboarding сохраняются; до их завершения форма предлагает соответствующую настройку с возвратом к приглашению.
