...
Границы расширены владельцем 2026-09-05: [принятые решения](PRODUCT_DECISIONS.md), [реализованные контуры и проверки](PRODUCT_IMPLEMENTATION.md). Ниже сохранена исходная проверяемая гипотеза и release gates бесплатного ядра. Новые вход/код пары, библиотека личного развития, структурированные знакомства, общая жизнь и экономика входят в текущую реализацию; прежнее исключение этих модулей из августовского MVP больше не задаёт объём работы. Локальный build и интеграции пройдены, проверка целевого Discord/Atlas окружения остаётся отдельным шагом.
