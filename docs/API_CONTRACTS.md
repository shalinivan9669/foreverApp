...
- `POST /api/pairs/[id]/events/[eventId]/accept` requires session auth, pair membership, and an idempotency key. It accepts `upcoming`, `offered`, and non-expired `snoozed` events for an active pair; an already `accepted` event is idempotent and returns existing generated activities. Accept runs the status transition, event-sourced `PairActivity` creation, and `generatedActivityIds` update in a MongoDB transaction. Event activity creation also checks `(pairId, stateMeta.sourceMeta.eventId, stateMeta.templateId)` and reuses an existing matching activity instead of creating a duplicate.
...
- `PairActivityDTO.eventSource` is an optional privacy-safe source marker for event-generated activities. When present, it includes `trigger: "pair_event"` plus optional event id/type/category/date for UI badges; it does not expose raw `stateMeta` or check-in/questionnaire answers.
...
- `POST /api/pairs/[id]/suggest` requires session auth, pair membership, entitlement/quota checks, and an idempotency key. It returns `{ plan, currentActivity, offers, createdCount, skippedReason? }`. The decision plan considers pair status, current activity, diagnostics confidence/risk zones, current weekly check-in submission, pair fatigue/readiness/closeness/irritation, weekly divergence, offered backlog, recent template history, and cooldowns. It creates at most three concurrent offers, never creates new offers while a current activity exists, and returns DTOs only.
- Activity suggestions persist decision context in `PairActivity.stateMeta` with `source`, `sourceMeta`, `primaryReason`, optional `axis`/`severity`/`weekKey`, and `decisionVersion: activity-decision-v1`. The shape is compatible with future `calendar_event` and `relationship_milestone` sources, but no event scheduler or calendar logic exists yet.
...
- `POST /api/activities/[id]/checkin` validates every effective feedback question, replaces the authenticated participant's previous answer for the same question, and returns aggregate submitted count only. A second participant may refine a one-sided `completed_partial` result from history without applying the activity effect twice.
- `POST /api/activities/[id]/complete` requires at least one feedback submission and returns `{ success, status, resultSummary }`. One-sided results are always preliminary `completed_partial`; two-sided low-comfort/high-tension results can be `failed`. Result effects are bounded, recorded with `resultVersion: activity-result-v1`, and activity vector snapshots atomically preserve `source: activity_completion`, the activity id, result revision, and scoring version. Legacy snapshots without the optional activity provenance fields remain readable.
...
- Pair suggestion plans include `recentActivitySignals`. Recent low comfort limits intensity, a failed format is deprioritized, high `wantsSimilarRatio` favors the same archetype, and normal template cooldown still prevents immediate duplicate offers.
