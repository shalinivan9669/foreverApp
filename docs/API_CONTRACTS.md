...
- `POST /api/pairs/[id]/events/[eventId]/accept` requires session auth, pair membership, and an idempotency key. It accepts an active/upcoming/snoozed event for an active pair, creates up to three event-sourced `PairActivity` offers without exceeding the existing offered/current activity limit, stores event linkage in `PairActivity.stateMeta.sourceMeta`, and returns `{ event, activities }`. Repeated accept returns existing generated activities rather than duplicating them.
- `POST /api/pairs/[id]/events/[eventId]/decline` and `POST /api/pairs/[id]/events/[eventId]/snooze` require session auth, pair membership, and an idempotency key. Decline hides the event without deleting it. Snooze accepts `{ days?: 1|3|7 }`, defaults to three days, and rejects snoozes that exceed the event window.
