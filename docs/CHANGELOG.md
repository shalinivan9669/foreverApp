...
Date: 2026-09-09
Summary:
- Реализованы шесть одноразовых измерительных анкет с продолжением черновика, неизменным финалом, каноническим retry и восстановлением вычислений после сбоя. Новая коллекция MeasurementTestSession, стабильный уникальный _id; новых зависимостей и env/auth изменений нет.
- Собственные характеристики показывают значение, шкалу, уверенность, свежесть и историю; private/pair/matching SELF-источники видны автору. Устойчивые характеристики сгруппированы по областям, состояния отделены.
- Разрешённые результаты потребляются Factor Matching и моделью действующей пары; отзыв согласия убирает общий вывод без удаления личного результата. Устаревшие grants/feed инвалидируются, поиск автоматически не включается.
...
Date: 2026-09-10
Summary:
- Added nine core private-beta K/D/A forms and one optional DOM application clarification, preserving the original publication and existing saved consent scope. Compatible owner episode references deduplicate real cross-publication roots; incompatible instruments and prior Pair contexts remain separate.
- Completed source composition, component history, durable source-set CAS, private negative correction retention, owner definitions/provenance, deterministic planner and persisted practices; practices do not create measured evidence.
- Added exact source coverage with 39 pure checks, 73 HTTP/Mongo assertions and 15 actual component/normalizer assertions, plus retained assessment regression. Recorded bounded internal content-copy review and updated API/source docs; external release gates remain separate.
Files: src/domain/assessment, src/domain/services/assessmentRuns.service.ts, src/models/AssessmentRun.ts, src/models/AssessmentPortfolio.ts, src/models/AssessmentPractice.ts, src/lib/dto/assessment.dto.ts, src/features/assessments, scripts/beta-content.selfcheck.ts, scripts/beta-content-profile-ui.selfcheck.ts, scripts/beta-sources.integration.ts, docs/BETA_SOURCES.md, docs/API_BETA_CONTRACTS.md, docs/assessment/BETA_SOURCES_COVERAGE.json.

Date: 2026-09-10
Summary:
- Completed the connected B01–B12 private-beta code scope: explicit adult admission and saved purpose settings; existing profile, bounded discovery and voluntary contact; current and conditional plans; authored recurring pair agreements with two independent confirmations and private/shared reports.
- Added durable workers, recovery fencing, independent revocation ledger, scheduled cleanup, guarded operator controls, synthetic migration/restore and bounded load acceptance. A verified archive of the actual retained release is executed to prove supported rollback with both current and legacy switches OFF; old code cannot restore revoked rights. Real target approval, OAuth/Discord review, native assistive-technology execution and actual cohort opening remain separate gates.
- Fixed account-switch and cancelled-preflight races, late-response ownership, pending form edits, local-date timezone round trips and invalid intermediate timezone input. Browser checks exercise native inputs, complete K/D/A forms, errors/retry, same-origin logout/login, native Chrome zoom and reduced-motion media.
- Added one reproducible acceptance command, local/CI configuration and exact assertion maps for all 126 BETA plus fresh re-execution of 64 retained INT requirements. Each run records byte-exact source identity and verifies its complete binary Git patch in separate indexes; previous results cannot certify a changed tree.
