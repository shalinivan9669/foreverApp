...
- [Тактильный интерфейс меню, тестов и личных анкет](UX_TACTILE_INTERFACE.md) — восстановленная крупная плитка, адаптивная компоновка, читаемость, физический отклик и локальная браузерная проверка.

## Доступ после регистрации — 2026-09-10

- [Анкеты без приглашений, единый вход, настройки и эксплуатация](ADR/ADR-010-registered-assessment-access.md) — актуальное решение владельца: beta означает стадию тестирования, а не список приглашённых. Заменяет прежнее требование OFF по умолчанию и обязательного оператора для обычных пользователей.
