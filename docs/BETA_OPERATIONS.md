...
> Актуальное решение владельца: обычное приложение использует REGISTERED по умолчанию и не требует приглашений или перечисленной ниже beta-конфигурации. См. [ADR-010](ADR/ADR-010-registered-assessment-access.md). Этот документ сохраняет команды для **явно выбранного PRIVATE_BETA** и независимого восстановления. Первичная установка REGISTERED создаёт индексы автоматически, не очищая пользователей.
