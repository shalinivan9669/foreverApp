...
Reference-проверка исходных каталогов сравнивает закреплённые SHA-256 после нормализации только CRLF → LF, соответствующей Git text normalization. Это позволяет одинаково проверять Linux checkout и Windows с `core.autocrlf=true`. Пробелы, формулировки, порядок и значения не нормализуются; исходные provenance hashes остаются прежними. Byte-exact identity конкретного тестового дерева по-прежнему записывается отдельно и не нормализуется.
