...
export default function PairDiagnosticsPage() {
  ...
  // Load a couple questionnaire id for CTA
  useEffect(() => {
    let on = true;
    fetch(api('/api/questionnaires?target=couple'))
      .then(r => r.ok ? r.json() : [])
      .then(list => { if (on) setCoupleQnId(list?.[0]?._id ?? null); })
      .catch(() => {});
    return () => { on = false; };
  }, []);
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(api(`/api/pairs/${pairId}/diagnostics`));
  ...
  const severityBadge = (n: 1|2|3) => (
    <span className={`px-2 py-0.5 rounded text-xs ${n===3?'bg-red-100 text-red-700':n===2?'bg-amber-100 text-amber-700':'bg-yellow-100 text-yellow-700'}`}>S{n}</span>
  );
  ...
  return (
    <main className="p-4 max-w-3xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/main-menu" className="text-sm text-blue-600 hover:underline">В меню</Link>
      </div>

      <div className="flex gap-2">
        <button onClick={refresh} disabled={loading} className="px-3 py-2 rounded bg-black text-white disabled:opacity-50">Обновить</button>
        {coupleQnId && (
          <Link href={ctaHref} className="px-3 py-2 rounded bg-yellow-300 text-black">Пройти диагностику</Link>
        )}
      </div>
      ...
      {!loading && passport && (
        <div className="space-y-4">
          ...
          <section className="space-y-2">
            ...
            {passport.strongSides?.length ? passport.strongSides.map((s, i) => (
          ...
          <section className="space-y-2">
            ...
            {passport.complementMap?.length ? passport.complementMap.map((c, i) => (
              <div key={`${c.axis}-${i}`} className="border rounded p-3 text-sm">
                <div className="font-medium">{c.axis}</div>
                <div className="text-gray-700">
                  A покрывает B: {c.A_covers_B?.length ? c.A_covers_B.join(', ') : '—'}
                </div>
                <div className="text-gray-700">
                  B покрывает A: {c.B_covers_A?.length ? c.B_covers_A.join(', ') : '—'}
                </div>
              </div>
