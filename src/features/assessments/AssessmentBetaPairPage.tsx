...
function Workspace({ data, busy, send, reload }: { data: Data; busy: boolean; send: Send; reload: () => Promise<void> }) {
  ...
  return <>
    {pair.availability !== 'AVAILABLE' ? <section className="app-panel p-4 space-y-3"><h2 className="font-semibold">Совместный контекст пока недоступен</h2><p>Нужны действующая пара, допуск и разрешённые собственные условия обоих участников. Прежняя пара и отозванные разрешения не используются автоматически.</p><Link className="app-btn-secondary inline-flex" href="/pair">Проверить состояние пары</Link><Link className="app-btn-secondary inline-flex" href="/assessments/conditions">Мои условия</Link><Link className="block underline" href="/profile/settings#assessment-settings">Настройки и управление данными</Link></section> : <>
...
export default function AssessmentBetaPairPage() {
  ...
  return <main className="app-shell-narrow py-5 space-y-5"><BackBar title="Договорённости пары" fallbackHref="/assessments" /><h1 className="text-2xl font-semibold">Договорённости пары</h1><p>Согласуйте конкретное действие, время и распределение работ. Каждый сохраняет право на отдельный взгляд, пересмотр и выход.</p><nav className="flex flex-wrap gap-3" aria-label="Условия и наблюдения"><Link className="app-btn-secondary" href="/assessments/conditions">Мои условия и ресурс</Link><Link className="app-btn-secondary" href="/assessments">Навыки и анкеты</Link></nav>
