...
export default function MatchingConnectionPage({ connectionId }: { connectionId: string }) {
  return <MatchingAccessGate><MatchingConnectionPageContent connectionId={connectionId} /></MatchingAccessGate>;
}
...
function MatchingConnectionPageContent({ connectionId }: { connectionId: string }) {
  ...
  return <main className="app-shell-narrow space-y-5 py-6">
    ...
    {currentConnection && <MatchingConnectionCard connection={currentConnection} loading={connection.actionLoading || api.loading} onAction={async (action) => { if (!confirmAppNavigation()) return false; const ok = await connection.confirm(action); if (ok) await load(); return ok; }} />}
    {accessibleConversation && conversation && <>
      ...
      {conversation.topics.map((topic) => <Topic key={`${connectionId}:${topic.topicKey}`} topic={topic} initiallyOpen={topic.topicKey === nextTopic?.topicKey} loading={api.loading || connection.actionLoading} canWrite={canWrite} mutate={mutate} />)}
