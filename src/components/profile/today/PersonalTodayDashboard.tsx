...
import { clampPercent } from '@/client/viewmodels';
    ...
    <header className="app-panel app-panel-solid p-4 sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="app-panel-soft app-panel-soft-solid w-full rounded-lg p-3 text-sm sm:max-w-xs">
          ...
          <div className="mt-1 font-semibold">{today.hero.title}</div>
        </div>
      </div>
      {open && (
...
function Ring({
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width="96" height="96" viewBox="0 0 96 96" aria-label={`${label} ${formatPercent(value)}`}>
...
function PersonalPulseRing({ today }: { today: PersonalTodayDTO }) {
  return (
    <section className="app-panel app-panel-solid p-4 sm:p-5">
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1.2fr_1fr] lg:items-center">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Ring value={today.hero.rings.resource} label="Ресурс" tone="#10b981" />
          <Ring value={today.hero.rings.closeness} label="Близость" tone="#f59e0b" />
          <Ring value={today.hero.rings.tension} label="Напряжение" tone="#ef4444" />
        </div>
      </div>
    </section>
  );
...
function PartnerSignalCard({
  ...
  const send = async () => {
    ...
    setStatus(null);
    try {
      await usersApi.sendPartnerSignal(today.checkIn.id, { text });
  ...
  return (
    <section className="app-panel app-panel-solid p-4">
      ...
      {status && <p className="app-muted mt-3 text-sm">{status}</p>}

      <div className="mt-4 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={send}
          disabled={sending || today.partnerSignal.visibility === 'sent'}
          className="app-btn-primary px-4 py-2 text-sm disabled:opacity-60"
        >
          {sending ? 'Отправляем...' : today.partnerSignal.primaryCta}
        </button>
        <button
          type="button"
...
function SoftOptionCard({
  ...
  return (
    <section className="app-panel app-panel-solid p-4">
      <h2 className="text-lg font-semibold">{today.softOption.title}</h2>
      <p className="app-muted mt-2 text-sm">{today.softOption.intro}</p>
      <div className="app-panel-soft app-panel-soft-solid mt-4 rounded-lg p-3 text-sm">
        {today.softOption.phrase}
      </div>
      {open && (
        <div className="mt-3 space-y-2">
          {today.softOption.alternatives.map((item) => (
            <button
              key={item}
              type="button"
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => onSelectPhrase(today.softOption.phrase)}
          className="app-btn-primary px-4 py-2 text-sm"
        >
          {today.softOption.primaryCta}
        </button>
        <button
          type="button"
          onClick={() => setOpen((value) => !value)}
  ...
  return (
    <section className="app-panel app-panel-solid p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <span className="app-muted text-xs">{text.length}/{today.privateJournal.maxLength}</span>
      </div>
      ...
      <textarea
        ...
        className="mt-4 min-h-32 w-full rounded-lg border border-black/10 bg-white/60 p-3 text-sm outline-none focus:border-[var(--app-accent,#8b5cf6)]"
      />
      <p className="app-muted mt-3 text-sm">
        {today.checkIn.submittedToday
          ? 'Дневник сохраняется вместе с отметкой состояния. Для изменения отправь отметку ещё раз в форме ниже.'
          : 'Сначала отметь состояние за сегодня, потом можно сохранить личную запись.'}
      </p>
    </section>
  );
}
...
function DailyCheckInCard({
  ...
  const [answers, setAnswers] = useState<PersonalDailyCheckInRequest['answers']>({
    mood: 'calm',
    energy: today.hero.rings.resource,
    stress: today.hero.rings.tension,
    closenessNeed: today.hero.rings.closeness,
    spaceNeed: 0.35,
    supportNeed: today.todayMap.find((item) => item.key === 'support')?.value ?? 0.5,
    conflictSensitivity:
      today.todayMap.find((item) => item.key === 'irritation')?.value ?? today.hero.rings.tension,
    conversationReadiness:
      today.todayMap.find((item) => item.key === 'conversation')?.value ?? 0.5,
  });
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {(Object.keys(sliderLabels) as SliderKey[]).map((key) => (
              <label key={key} className="app-panel-soft app-panel-soft-solid rounded-lg p-3 text-sm">
                <span className="flex items-center justify-between gap-2">
                  <span>{sliderLabels[key]}</span>
                  <span className="app-muted">{formatPercent(answers[key])}</span>
                </span>
                <input
                  type="range"
                  min="0"
          ...
          {message && <p className="app-muted text-sm">{message}</p>}
          <button
            type="button"
            onClick={submit}
            disabled={saving}
    ...
    () => (
      <div className="app-dashboard-grid">
        <div className="app-grid-wide">
          <PartnerSignalCard today={today} draftText={draftText} onRefresh={onRefresh} />
        </div>
        <div className="app-grid-narrow">
          <SoftOptionCard today={today} onSelectPhrase={setDraftOverride} />
        </div>
      </div>
    ),
    [draftText, onRefresh, today]
  );

  return (
    <section className="app-page-stack">
      <PersonalTodayHeader today={today} />
      <PersonalPulseRing today={today} />
