    ...
    <div className="space-y-3">
      {activity && (
        <div className="app-panel app-panel-solid p-4">
          <div className="font-display text-lg font-semibold leading-tight">{activity.title ?? 'Текущая активность'}</div>
      ...
      {suggested?.map((entry) => (
        <div key={entry.id} className="app-panel-soft app-panel-soft-solid p-4">
          <div className="font-display text-lg font-semibold leading-tight">{entry.title ?? 'Предложено'}</div>
        </div>
      ))}
    </div>
  );
