  ...
  return (
    <div className="rounded border p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500">{a.axis.join(', ')}</span>
          ...
          {a.description && <p className="text-sm text-gray-600 mt-1">{t(a.description)}</p>}
          {a.timeEstimateMin ? (
            <div className="text-xs text-gray-500 mt-1">~{a.timeEstimateMin} мин</div>
          ) : null}
          {a.dueAt ? (
            <div className="text-xs text-gray-500">до {new Date(a.dueAt).toLocaleString()}</div>
          ) : null}
        </div>
      </div>
      ...
      <div className="mt-3 flex gap-2">
        {variant === 'suggested' && (
          <>
            <button onClick={onAccept} className="px-3 py-2 rounded bg-black text-white">Принять</button>
            <button onClick={onCancel} className="px-3 py-2 rounded border">Отклонить</button>
          </>
        )}

        {variant === 'active' && (
          <>
            <button onClick={onComplete} className="px-3 py-2 rounded bg-black text-white">Завершить</button>
            <button onClick={onCancel} className="px-3 py-2 rounded border">Отменить</button>
            <div className="flex-1" />
            <button onClick={onSuggestNext} className="px-3 py-2 rounded border">Предложить ещё</button>
          </>
        )}
