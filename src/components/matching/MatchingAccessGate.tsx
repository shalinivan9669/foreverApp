...
export default function MatchingAccessGate({ children, requireCard = false }: { children: ReactNode; requireCard?: boolean }) {
  ...
  const userId = access.allowed ? access.user?.id : undefined;
  ...
  const nextHref = entryRequired ? "/entry?intent=matching" : access.allowed ? redirectHref : null;
  ...
  if ((access.error || prerequisiteError) && (!access.verified || access.eligibility === "ELIGIBLE" || entryRequired)) {
  ...
  if (access.verified && !access.allowed && !entryRequired) return <main className="app-shell-narrow py-6"><section className="app-panel p-5" role="status"><h1 className="text-xl font-semibold">{access.eligibility === "ADULT_REQUIRED" ? "Знакомства доступны с 18 лет" : "Ваш маршрут — отношения с партнёром"}</h1><p className="app-muted mt-3">{access.eligibility === "PAIR_ACTIVE" ? "У вас уже есть пара. Поиск новых партнёров и новые знакомства недоступны, в том числе во время паузы пары." : access.eligibility === "EXISTING_PARTNER" ? "Вы указали, что у вас есть партнёр. Поиск новых знакомств недоступен и до связывания аккаунтов. Продолжайте путь со своим партнёром или выбирайте личное развитие." : "Продолжить личное развитие и открыть настройки можно из главного меню."}</p><div className="mt-4 flex flex-wrap gap-2"><Link className="app-btn-primary" href={access.eligibility === "EXISTING_PARTNER" ? "/invite" : "/main-menu"}>{access.eligibility === "EXISTING_PARTNER" ? "Пригласить своего партнёра" : "Главное меню"}</Link><Link className="app-btn-secondary" href="/development">Личное развитие</Link><Link className="app-btn-secondary" href="/profile/history">Моя история</Link><Link className="app-btn-secondary" href="/profile/safety">Безопасность</Link></div></section></main>;
  ...
  return <MatchingAccessContext.Provider value={access.refresh}><div aria-busy={access.checking} inert={access.checking || undefined}>{children}</div></MatchingAccessContext.Provider>;
}
