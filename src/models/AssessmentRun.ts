...
import type { AssessmentAnswerRecord, AssessmentExposureHistory, AssessmentPeriod } from '@/domain/assessment/contracts';
...
export type AssessmentRunType = {
  ...
  contentHash?: string; contextIdentity?: string; goal?: 'SELF' | 'DATING' | 'COUPLE';
  ...
  previousSource: { revision: number; period: AssessmentPeriod; answers: AssessmentAnswerRecord[]; snapshot: AssessmentProfileSnapshot; observationRound: number; finalizedAt: string; contextIdentity?: string } | null;
  followupSince: string | null;
  finalizedAt: string | null; createdAt: Date; updatedAt: Date;
  sourceHistory?: Array<{ revision: number; period: AssessmentPeriod; answers: AssessmentAnswerRecord[]; observationRound: number; finalizedAt: string; contextIdentity?: string }>;
};
...
const schema = new Schema<AssessmentRunType>({
  ...
  contentHash: { type: String }, contextIdentity: { type: String, default: 'INDIVIDUAL' }, goal: { type: String, enum: ['SELF', 'DATING', 'COUPLE'], default: 'SELF' },
  exposureHistory: { type: String, enum: ['NOT_RECORDED', 'EXPLANATION_RECORDED', 'UNKNOWN_AFTER_DELETION'] },
  ...
  sourceHistory: { type: Schema.Types.Mixed, default: [] },
}, { collection: 'assessment_runs', versionKey: false, timestamps: true });
schema.index({ ownerId: 1, status: 1 }, { name: 'assessment_owner_sources' });
schema.index({ ownerId: 1, publicationId: 1, publicationVersion: 1 }, { name: 'assessment_owner_publication', unique: true });
