...
const contentFor = (key: string) => findDevelopmentContent(key) ?? missing();
const contentForRun = (run: {
  contentKey: string;
  contentRevision: number;
}) => {
  const content = contentFor(run.contentKey);
  if (content.revision !== run.contentRevision)
    throw new DomainError({
      code: "CONTENT_VERSION_CHANGED",
      status: 409,
      message:
        "Материал обновлён. Откройте новое прохождение из библиотеки; прежний результат сохранён без переоценки.",
    });
  return content;
};
...
export const developmentService = {
  ...
  async overview(userId: string): Promise<DevelopmentOverviewDTO> {
    await connectToDatabase();
    const [completions, runs, reflections, accessiblePairs] = await Promise.all(
      [
        DevelopmentCompletion.aggregate<{ _id: string; count: number }>([
          { $match: { userId } },
          { $group: { _id: "$contentKey", count: { $sum: 1 } } },
        ]),
        DevelopmentRun.find({ participantIds: userId })
          .sort({ createdAt: -1 })
          .limit(30)
          .lean(),
        DevelopmentCompletion.find({ userId, "answers.0": { $exists: true } })
          ...
          .limit(20)
          .lean(),
        Pair.find({ members: userId, status: { $in: ["active", "paused"] } })
          .select({ _id: 1 })
          .lean(),
      ],
    );
    ...
    const focus = reflections.find((reflection) =>
      reflection.answers.some(
        (answer) => answer.value !== null && answer.value <= 1,
      ),
    );
    const focusDomain = focus ? contentFor(focus.contentKey).domain : null;
    const next =
      DEVELOPMENT_CATALOG.find(
        (item) =>
    ...
    const pairIds = new Set(accessiblePairs.map((pair) => pair._id.toString()));
    return {
      suggestion: {
        contentKey: next.key,
        title: next.title,
      ...
      content: DEVELOPMENT_CATALOG.map((card) => ({
        key: card.key,
      ...
      recent: runs
        .filter((run) => !run.pairId || pairIds.has(run.pairId))
  ...
  async detail(userId: string, runId: string): Promise<DevelopmentDetailDTO> {
    ...
    return {
      content: contentForRun(run),
      run: toDevelopmentRunDTO(run, userId),
      responseOptions: REFLECTION_OPTIONS,
      ownResult: own
        ? {
  ...
  async complete(
    ...
    const content = contentForRun(run);
    ...
    try {
      await session.withTransaction(async () => {
        ...
        current.status = current.participantIds.every((id) =>
          ...
          ? "COMPLETED"
          : "PARTIAL";
        if (current.status === "COMPLETED") current.completedAt ??= new Date();
        await current.save({ session });
        if (!run.pairId) {
          await economyService.rewardCompletion({
            userId,
            sourceKind:
              content.kind === "REFLECTION" ? "QUESTIONNAIRE" : "SOLO_PRACTICE",
