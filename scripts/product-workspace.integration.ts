...
async function main() {
  ...
  assert.equal(newRunAfterPublication.run.id, input.runId);
  assert.equal(newRunAfterPublication.content.revision, 1, 'a new publication must not reopen a completed reflection');
  assert.deepEqual(newRunAfterPublication.content.prompts, originalPublication.content.prompts);
  assert.deepEqual(newRunAfterPublication.responseOptions, originalPublication.responseOptions);
  const freshPublicationRun = await updatedService.start(outsider, 'communication.reflection.1');
  assert.equal(freshPublicationRun.content.revision, 2);
  assert.deepEqual(freshPublicationRun.content.prompts, updatedPublication.content.prompts);
  assert.deepEqual(freshPublicationRun.responseOptions, updatedPublication.responseOptions);
  await assert.rejects(updatedService.complete(outsider, { ...input, runId: freshPublicationRun.run.id }), (error: Error) => error instanceof DomainError && error.code === 'VALIDATION_ERROR');
