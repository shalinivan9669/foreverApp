...
const declaredTypeNode = (path: string, declarationName: string): ts.TypeNode => {
...
const typeMember = (
  ...
  assert.ok(ts.isTypeLiteralNode(literal), `${memberName} must be in a type literal`);
...
const variableObjectProperties = (
  ...
  const declaration = sourceFile(path).statements
  ...
  assert.ok(declaration?.initializer, `${path} must initialize ${variableName}`);
...
for (const path of [
  ...
  assertTypeOmits(path, 'RecommendationDecisionDTO', ['targetFactorKeys'], [
    'activity',
  ]);
}
assert.equal(
...
assert.ok(
  WeeklyCheckIn.schema.indexes().some(
    ([keys, options]) =>
      options.unique === true &&
      JSON.stringify(keys) ===
  ...
  ['member-a', 'member-b'].map((userId, index) => ({
    ...
    computed: {
      factorEngine: {
        status: 'MATERIALIZED',
        evidenceEventIds:
          privateVariant === 'low' ? ['private-evidence-low'] : ['private-evidence-high'],
        individualSnapshotIds:
          privateVariant === 'low'
...
const pairEvaluation = (
  ...
  snapshotId: variant === 'left' ? 'PRIVATE_PAIR_EVAL_LEFT' : 'PRIVATE_PAIR_EVAL_RIGHT',
  pairId: 'pair-1',
...
assert.equal(
  disclosePairEvaluation(pairEvaluation('left'), 'PAIR_MEMBER', false).disclosure,
  'WITHHELD'
);
...
assertSerializedOmits('activity DTO', activityDtoLeft, INTERNAL_ACTIVITY_FIELDS);
...
const pairEvent = (
  ...
  pairId: new Types.ObjectId('507f1f77bcf86cd799439011'),
  key: variant === 'left' ? 'PRIVATE_EVENT_KEY_LEFT' : 'PRIVATE_EVENT_KEY_RIGHT',
...
assertNoSecrets('pair event DTO', pairEventDtoLeft, ['PRIVATE_EVENT_', 'PRIVATE_FACTOR_', 'PRIVATE_SOURCE_']);
...
const pairSummary = (variant: 'left' | 'right'): PrivatePairSummaryInput => ({
  ...
  lastLike: {
    ...
    note: variant === 'left' ? 'PRIVATE_LAST_LIKE_LEFT' : 'PRIVATE_LAST_LIKE_RIGHT',
  },
  progress: {
    streak: variant === 'left' ? 1 : 999,
    completed: variant === 'left' ? 2 : 998,
...
type PrivateHistoryCycle = Parameters<typeof projectCanonicalHistoryCycle>[0] & {
    ...
    signals: [
      {
        ...
        status: 'STEADY',
        reasonCode:
          variant === 'left' ? 'PAIR_LEVEL_LOW' : 'PAIR_LEVEL_HIGH',
        nextStepHint:
...
const notification = (
  ...
  userId: variant === 'left' ? 'PRIVATE_NOTIFICATION_USER_LEFT' : 'PRIVATE_NOTIFICATION_USER_RIGHT',
...
assertNoSecrets('notification DTO', notificationLeft, ['PRIVATE_NOTIFICATION_']);
...
const partnerSignal = (
  ...
  updatedAt: new Date('2026-08-11T12:00:00.000Z'),
  computedMode:
    variant === 'left' ? 'PRIVATE_MODE_LEFT' : 'PRIVATE_MODE_RIGHT',
});
...
for (const path of participantBoundaryFiles) {
  ...
  assert.equal(importsSafetyGate, false, `${path} imports owner-private SafetyGate`);
}
